...
public class Individual_128184 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (16.89460277557373)*en.energy;
				eval += (63.51884603500366)*myEnergy;
				eval += (56.01056218147278)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (25.510483980178833)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
