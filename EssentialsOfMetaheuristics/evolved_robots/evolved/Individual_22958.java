...
public class Individual_22958 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (50.564658641815186)*en.energy;
				eval += (45.78951597213745)*myEnergy;
				eval += (49.033886194229126)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (3.5460054874420166)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
