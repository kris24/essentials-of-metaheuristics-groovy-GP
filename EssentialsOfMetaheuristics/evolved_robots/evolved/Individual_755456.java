...
public class Individual_755456 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (83.27913880348206)*en.energy;
				eval += (42.710745334625244)*myEnergy;
				eval += (4.516690969467163)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (50.721460580825806)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
