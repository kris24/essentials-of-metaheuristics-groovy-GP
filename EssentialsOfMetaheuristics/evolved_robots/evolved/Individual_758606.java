...
public class Individual_758606 extends AdvancedRobot
{
...
//- eval position ---------------------------------------------------------------------------------------------------------------------------
	public static double evaluate(Point2D.Double p, double addLast) {
		...
		while (enemy_enum.hasMoreElements()) {
			...
			if(en.live) {
				eval += (62.5720739364624)*en.energy;
				eval += (39.14394974708557)*myEnergy;
				eval += (58.80901217460632)*(calcAngle(myPos, p) - calcAngle(en.pos, p));
				eval += (80.42080998420715)*p.distanceSq(en.pos);
			}
		}
		return eval;
	}
